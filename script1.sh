...
ssh -i "kplabs-terraform.pem" ec2-user@ec2-34-234-203-145.compute-1.amazonaws.com
#check if the webserver is running & and serving the expected content if not start it
#/usr/bin/wget "http://54.209.117.2/" --timeout 30 -O - 2>/dev/null | grep "nginx" && echo "webserver is alive and content verified"  || sudo systemctl start nginx
/usr/bin/wget "http://34.234.203.145/" --timeout 30 -O - 2>/dev/null | grep "nginx" && echo "webserver is alive and content verified"  || sudo systemctl start nginx
